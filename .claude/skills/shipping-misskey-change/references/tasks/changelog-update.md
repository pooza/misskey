...
⚠ **フォーク独自機能 (`WidgetTagset` など upstream に存在しないもの) だけの変更では追記しない。** `CHANGELOG.md` は upstream 所有のファイルで、追従のたびに `## Unreleased` が `## <version>` へ畳まれる。フォークのエントリを置くと毎回の追従で衝突し、upstream のリリースノートに紛れ込む。このフォークは独自のリリースノートを publish していない (実際、フォーク側のコミットで `CHANGELOG.md` を編集したものは 1 件も無い)。upstream にも影響する変更 (upstream へ PR を出す前提のもの) は従来どおり追記する。
