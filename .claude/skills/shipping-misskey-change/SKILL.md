...
`scripts/check-shipping.mjs` は upstream 由来 (2026.9.0 で入った) で、**このフォークの事情を 2 点知らない**。

- **base ref**: 既定の統合先は `origin/develop` / `develop` / `origin/master` / `master` の順で、
  このフォークのベースブランチ `daisskey` が入っていない。`develop` は upstream 追従用のミラーなので、
  黙って実行すると**フォーク独自変更が丸ごと「変更ファイル」に化ける**。
  必ず `node scripts/check-shipping.mjs --base daisskey` (または `MISSKEY_BASE_REF=daisskey`) を使う
- **locale safety**: `locales/ja-JP.yml` 以外の locale YAML 変更を無条件で `FAIL` にする。
  ⚠ **フォーク独自キー (`_tagset` 等) を `locales/en-US.yml` へ手で足すのは正当** (理由は
  [AGENTS.md](../../../AGENTS.md) の「絶対にやってはいけない事」#2 の例外)。
  `forbidden locale change: locales/en-US.yml` **だけ**が出ていて差分が独自キーに閉じているなら合格とみなす。
  それ以外の locale ファイルが出たら止める
